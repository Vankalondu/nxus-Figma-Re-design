      ...
      <PopoverContent className="w-[300px]">
        ...
        <div className="flex flex-wrap gap-2 mb-3">
          {['Wrong player', 'Poor quality', 'Too short'].map((c) => (
            <span key={c} className="px-3 py-1 rounded-full bg-surface-accent text-body font-body font-bold text-[10px] cursor-pointer hover:text-body">
              {c}
            </span>
          ))}
        </div>
        <textarea
          ...
          className="w-full bg-surface-card border border-default rounded-xl px-3 py-2 text-[12px] text-body focus:outline-none focus:ring-2 focus:ring-border-focus/20"
        />
      </PopoverContent>
    </Popover>
  ),
