    ...
    <ScrollArea className="h-[180px] w-[320px] rounded-[20px] border border-default p-4">
      <div className="flex flex-col gap-3">
        {Array.from({ length: 14 }, (_, i) => (
          <span key={i} className="font-body text-[12px] text-body">
    ...
    <ScrollArea className="w-[360px] rounded-[20px] border border-default">
      <div className="flex gap-4 p-4">
        {Array.from({ length: 10 }, (_, i) => (
          <div key={i} className="w-[120px] h-[80px] shrink-0 rounded-[16px] bg-surface-accent flex items-center justify-center font-body text-[12px] text-body">
            Col {i + 1}
          </div>
        ))}
      </div>
