      ...
      <div className="bg-surface-card rounded-[20px] shadow-2xl w-full max-w-md border border-default" onClick={e => e.stopPropagation()}>
        <div className="px-8 py-6 bg-brand-primary rounded-t-[20px] flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            <Film size={18} className="text-on-brand" />
            <span className="font-heading font-semibold text-[16px] text-on-brand">Upload highlight</span>
          </div>
        ...
        <div className="p-8 space-y-4">
          <div className="flex items-center gap-2 font-body text-[13px] text-body">
            For <span className="font-bold text-strong">{playerName}</span>
          </div>
          ...
          <div className="flex items-center gap-1 p-1 bg-surface-card border border-default rounded-full w-full">
            {([['link', 'Paste link', Link2], ['file', 'Upload file', UploadCloud]] as const).map(([id, label, Icon]) => (
              <button key={id} onClick={() => setMode(id)}
                className={`flex-1 inline-flex items-center justify-center gap-1.5 font-body font-bold text-[12px] px-3 py-1.5 rounded-full transition-colors ${mode === id ? 'bg-brand-primary text-inverse shadow-sm' : 'text-body hover:text-strong'}`}>
                <Icon size={13} /> {label}
              </button>
            ))}
          </div>
          ...
          {mode === 'link' ? (
            ...
            <div
              ...
              {file ? (
                <div className="flex items-center gap-2 font-body font-bold text-[13px] text-body">
