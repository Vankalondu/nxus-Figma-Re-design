      ...
      <div className="bg-surface-card rounded-[20px] shadow-2xl w-full max-w-md border border-border-default" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-8 space-y-4">
          <div className="flex items-center gap-2 font-body text-[13px] text-text-body">
            For <span className="font-bold text-text-strong">{playerName}</span>
          </div>
          ...
          <div className="flex items-center gap-1 p-1 bg-surface-card border border-border-default rounded-full w-full">
            {([['link', 'Paste link', Link2], ['file', 'Upload file', UploadCloud]] as const).map(([id, label, Icon]) => (
              <button key={id} onClick={() => setMode(id)}
                className={`flex-1 inline-flex items-center justify-center gap-1.5 font-body font-bold text-[12px] px-3 py-1.5 rounded-full transition-colors ${mode === id ? 'bg-brand-primary text-text-inverse shadow-sm' : 'text-text-body hover:text-text-strong'}`}>
                <Icon size={13} /> {label}
              </button>
            ))}
          </div>
          ...
          {mode === 'link' ? (
            ...
            <div
              ...
              {file ? (
                <div className="flex items-center gap-2 font-body font-bold text-[13px] text-text-body">
                  <Check size={15} className="text-brand-primary" /> {file.name}
                </div>
              ) : (
                <>
                  <UploadCloud size={22} className="text-text-body" />
                  <div className="font-body font-bold text-[13px] text-text-body">Drop a video here, or click to choose</div>
